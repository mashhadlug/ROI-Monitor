...
class Field(object):
    ...
    def __init__(self, label=None, validators=None, filters=tuple(),
        ...
        self.title = label if label is not None else self.gettext(
            _name.replace('_', ' ').title()) or self.name
        self.label = Label(self.id, self.title)
...
class SelectField(SelectFieldBase):
    ...
    def __init__(self, label=None, validators=None,
                 coerce=text_type, choices=None, **kwargs):
...
class SelectMultipleField(SelectField):
    ...
    def iter_choices(self):
        for value, label in self.choices:
            selected = self.data is not None and self.coerce(
                value) in self.data
...
class FormField(Field):
    ...
    def __init__(self,
                 form_class, label=None, validators=None, separator='-', **kwargs):
...
class FieldList(Field):
    ...
    def __init__(
        ...
        assert isinstance(
            unbound_field, UnboundField), 'Field must be unbound, not a field class'
